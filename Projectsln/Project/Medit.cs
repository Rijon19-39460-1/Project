...
namespace Project
{
    public partial class Medit : Form
    {
        ...
        private void btnnext_Click(object sender, EventArgs e)
        {
            ...
            if (user == LManager.SetValueForText2)
            {
                string query = string.Format("select * from Manager where Username='{0}' ", user);
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            SqlConnection conn = new SqlConnection(connString);
            conn.Open();
            string query = string.Format("update Manager set Fname='{0}',Lname='{1}',Full_name='{2}',Gender='{3}',DOB='{4}',Nationality='{5}',Address='{6}',Phone='{7}',Email='{8}' where Id={9}", fname, lname, fullname, gender, dob, nation, address, phn, email, id);
            try
            {
                SqlCommand cmd = new SqlCommand(query, conn);
        ...
        private void btndlt_Click(object sender, EventArgs e)
        {
            ...
            string query = string.Format("delete from Manager where Id= " + id);
